...
class MovimientosMaterialesRepository:
    ...
    def delete(self, movimiento_id: int) -> None:
        ...
        """
        self._con.execute("UPDATE movimientos_materiales SET activo = 0 WHERE id = ?", (movimiento_id,))
        self._con.commit()
    ...
    def list_by_material(
        ...
        *,
        ...
        sql = "SELECT * FROM movimientos_materiales WHERE " + " AND ".join(clauses) + " ORDER BY fecha_hora DESC"

        try:
            rows = self._con.execute(sql, params).fetchall()
        except sqlite3.Error as exc:
            logger.error("Error SQL en MovimientosMaterialesRepository.list_by_material: %s", exc)
            return []
    ...
    def list_by_tipo(
        ...
        *,
        ...
        try:
            ...
            logger.error("Error SQL en MovimientosMaterialesRepository.list_by_tipo: %s", exc)
            return []
        return [self._row_to_model(r) for r in rows]
