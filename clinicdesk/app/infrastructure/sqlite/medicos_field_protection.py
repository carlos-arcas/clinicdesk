...
class MedicosFieldProtection:
    def __init__(self, connection: sqlite3.Connection) -> None:
        self._enabled = medicos_field_crypto_enabled()
...
def _has_crypto_columns(connection: sqlite3.Connection) -> bool:
    columns = {row["name"] for row in connection.execute("PRAGMA table_info(medicos)").fetchall()}
    required = {
        "documento_enc",
        "documento_hash",
        "email_enc",
