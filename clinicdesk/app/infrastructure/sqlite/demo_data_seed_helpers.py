...
def seed_recetas_dispensaciones(
    ...
    for idx in range(recipes):
        ...
        receta_cur = connection.execute(
            ...
            VALUES (?, ?, ?, ?, 'ACTIVA', 1)""",
            (
                patient_ids[idx % len(patient_ids)],
                doctor_ids[idx % len(doctor_ids)],
...
def seed_movimientos(
    connection: sqlite3.Connection, n_movimientos: int, staff_ids: list[int], seed: int, has_recetas: bool
) -> tuple[int, int]:
    ...
    for idx in range(total_med):
        ...
        connection.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, 1)""",
            (
                med_ids[idx % len(med_ids)],
                ts.isoformat(sep=" ", timespec="seconds"),
                "ENTRADA" if idx % 5 else "SALIDA",
                rng.randint(1, 20),
                "Movimiento demo",
                staff_ids[idx % len(staff_ids)] if staff_ids else None,
                "seed-demo" if has_recetas else "seed",
            ),
        )
    for idx in range(total_mat):
        ...
        connection.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, 1)""",
            (
                mat_ids[idx % len(mat_ids)],
                ts.isoformat(sep=" ", timespec="seconds"),
                "ENTRADA" if idx % 4 else "SALIDA",
                rng.randint(1, 15),
                "Movimiento demo",
                staff_ids[idx % len(staff_ids)] if staff_ids else None,
                "seed-demo",
...
def seed_turnos_y_calendario(
    connection: sqlite3.Connection, doctor_ids: list[int], staff_ids: list[int], months: int
) -> int:
    defaults = [("Mañana", "08:00", "14:00"), ("Tarde", "14:00", "20:00")]
    turnos_ids: list[int] = []
    for nombre, ini, fin in defaults:
        cur = connection.execute(
            "INSERT OR IGNORE INTO turnos (nombre, hora_inicio, hora_fin, activo) VALUES (?, ?, ?, 1)",
            (nombre, ini, fin),
        )
        if cur.lastrowid:
    ...
    for offset in range(days):
        ...
        for mid in doctor_ids:
            connection.execute(
                "INSERT OR IGNORE INTO calendario_medico (medico_id, fecha, turno_id, activo) VALUES (?, ?, ?, 1)",
                (mid, d.isoformat(), turnos_ids[(mid + offset) % len(turnos_ids)]),
            )
        for pid in staff_ids:
            connection.execute(
                "INSERT OR IGNORE INTO calendario_personal (personal_id, fecha, turno_id, activo) VALUES (?, ?, ?, 1)",
                (pid, d.isoformat(), turnos_ids[(pid + offset) % len(turnos_ids)]),
            )
    connection.commit()
...
def seed_ausencias(
    connection: sqlite3.Connection, doctor_ids: list[int], staff_ids: list[int], total: int, seed: int
) -> int:
    ...
    for idx in range(max(1, total - created)):
        ...
        connection.execute(
            ...
            (
                staff_ids[idx % len(staff_ids)],
                ini.isoformat(),
                fin.isoformat(),
                tipos[(idx + 1) % len(tipos)],
                "Ausencia demo",
                staff_ids[(idx + 1) % len(staff_ids)] if staff_ids else None,
                datetime.now().isoformat(sep=" ", timespec="seconds"),
            ),
        )
        created += 1
