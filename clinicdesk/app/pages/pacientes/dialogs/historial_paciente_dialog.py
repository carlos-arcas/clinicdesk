...
class HistorialPacienteDialog(QDialog):
    ...
    def _render_recetas(self, recetas: tuple[RecetaResumen, ...]) -> None:
        ...
        for receta in recetas:
            ...
            self.table_recetas.setItem(row, 0, _item(receta.fecha))
            self.table_recetas.setItem(row, 1, _item(receta.medico))
            self.table_recetas.setItem(row, 2, _item(receta.estado))
            self.table_recetas.setItem(row, 3, _item(str(receta.num_lineas)))
            activa = self._i18n.t("comun.si") if receta.activa else self._i18n.t("comun.no")
            self.table_recetas.setItem(row, 4, _item(activa))
    ...
    def _actualizar_lineas_receta(self) -> None:
        ...
        for linea in lineas:
            ...
            self.table_lineas.setItem(row, 0, _item(linea.medicamento))
            self.table_lineas.setItem(row, 1, _item(linea.posologia))
            self.table_lineas.setItem(row, 2, _item(linea.inicio))
            self.table_lineas.setItem(row, 3, _item(linea.fin))
            self.table_lineas.setItem(row, 4, _item(linea.estado))
