...
def normalizar_error_entrenamiento(err: str | BaseException | None) -> ErrorEntrenamientoNormalizado:
    ...
    if reason_code in {"dataset_insuficiente", "dataset_empty"}:
        return ErrorEntrenamientoNormalizado(
            reason_code=reason_code, mensaje_i18n_key="prediccion.entrenar.error.faltan_citas"
        )
    return ErrorEntrenamientoNormalizado(
        reason_code=reason_code, mensaje_i18n_key="prediccion.entrenar.error.no_preparar"
    )
