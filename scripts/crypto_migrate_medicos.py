...
LOGGER = logging.getLogger("scripts.crypto_migrate_medicos")
...
def parse_args() -> MigrationOptions:
    parser = argparse.ArgumentParser(description="Backfill seguro de cifrado por campo en médicos")
...
def _migrate(con: sqlite3.Connection, wipe_legacy: bool) -> MigrationStats:
    protection = MedicosFieldProtection(con)
    if not protection.has_columns:
        raise RuntimeError("La tabla medicos no tiene columnas *_enc/_hash")
...
def _migrate_row(
    ...
    *,
    row: sqlite3.Row,
    protection: MedicosFieldProtection,
    wipe_legacy: bool,
...
def _nullable_columns(con: sqlite3.Connection) -> set[str]:
    rows = con.execute("PRAGMA table_info(medicos)").fetchall()
...
def _execute_update(con: sqlite3.Connection, *, row_id: int, updates: dict[str, str | None]) -> None:
    ...
    con.execute(f"UPDATE medicos SET {assignments} WHERE id = ?", (*values, row_id))
